...
class ContrastiveLearner(acme.core.Learner):
    ...
    def __init__(
        ...
        def critic_loss(q_params, policy_params, target_q_params, transitions, key):
            ...
            if config.use_td:
                ...
                next_s, _ = jnp.split(
                    transitions.next_observation, [config.obs_dim], axis=1
                )
                if config.add_mc_to_td:
            ...
            if config.use_td:
                ...
                if config.add_mc_to_td:
                    loss = (
                        (1 + (1 - config.discount)) * loss_pos
                        + config.discount * loss_neg1
                        + 2 * loss_neg2
                    )
                else:
                    loss = (
                        (1 - config.discount) * loss_pos
                        + config.discount * loss_neg1
                        + loss_neg2
                    )
        ...
        def actor_loss(
            ...
            if config.use_gcbc:
                ...
                if config.bc_coef > 0:
                    ...
                    actor_loss = (
                        config.bc_coef * bc_loss + (1 - config.bc_coef) * actor_loss
                    )

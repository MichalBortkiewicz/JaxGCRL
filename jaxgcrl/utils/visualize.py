...
def visualize_sac_q_function_2d(policy_network, q_network, normalizer_params, policy_params, q_params,
                                state, goal_indices, x_bounds, y_bounds, wandb_key, 
                                grid_resolution=100):
    '''Visualize SAC Q-function as a heatmap over 2D goal space with policy-generated actions.
    - policy_network: SAC policy network
    - q_network: SAC Q network
    - normalizer_params: normalizer parameters
    - policy_params: policy network parameters
    - q_params: Q network parameters
    - state: (state_dim,) array - the state to condition on
    ...
    # Sample actions from policy for each goal (SAC outputs distribution, take mean)
    action_dist = policy_network.apply(normalizer_params, policy_params, obs_batch)
    actions = action_dist.mode()  # Use mode for deterministic action
    ...
    ax.set_xlabel('Goal x')
    ax.set_ylabel('Goal y')
    ax.set_title(f'SAC Q-Function Landscape\nState: [{state_goal_pos[0]:.2f}, {state_goal_pos[1]:.2f}]')
    ax.legend()
    ax.grid(True, alpha=0.3)

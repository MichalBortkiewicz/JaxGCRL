...
class AntMaze(d4rl.locomotion.ant.AntMazeEnv):
    ...
    def __init__(self, map_name, non_zero_reset=False):
        ...
        self.observation_space = gym.spaces.Box(
            low=np.full((58,), -np.inf), high=np.full((58,), np.inf), dtype=np.float32
        )
    ...
    def reset(self):
        ...
        state = state._replace(qpos=np.concatenate([goal_xy, state.qpos[2:]]))
        self.sim.set_state(state)
        for _ in range(50):
            self.do_simulation(np.zeros(8), self.frame_skip)
        self._goal_obs = self.BASE_ENV._get_obs(
            self

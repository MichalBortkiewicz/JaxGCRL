...
class ArmPickplaceEasy(PipelineEnv):
    def __init__(self, backend="mjx", **kwargs):
        ...
        if backend == "mjx":
            ...
            self.n_frames = 4
            self.episode_length = 200 # Merely a recommendation + used for scaling timestep
        elif backend == "positional":
            sys = mjcf.load("envs/assets/panda_pickplace_easy_positional.xml")
            sys = sys.tree_replace({
                "opt.timestep": 0.0003,
                "opt.iterations": 50,
            ...
            self.n_frames = 10
            self.episode_length = 2000 # Merely a recommendation + used for scaling timestep
        else:
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        state.metrics.update(success=reward, success_easy=jnp.array(dist < 0.15, dtype=float), success_hard=jnp.array(dist < 0.03, dtype=float))

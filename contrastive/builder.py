...
class ContrastiveBuilder(builders.ActorLearnerBuilder):
    ...
    def make_actor(self, random_key, policy_network, adder=None, variable_source=None):
        ...
        if self._config.use_random_actor:
            ACTOR = (
                contrastive_utils.InitiallyRandomActor
    ...
    def make_replay_tables(
        ...
        min_replay_traj = (
            self._config.min_replay_size // self._config.max_episode_steps
        )  # pylint: disable=line-too-long
        max_replay_traj = (
            self._config.max_replay_size // self._config.max_episode_steps
        )  # pylint: disable=line-too-long
    ...
    def make_dataset_iterator(self, replay_client):
        ...
        dataset = dataset.map(
            add_info_fn, num_parallel_calls=tf.data.AUTOTUNE, deterministic=False
        )

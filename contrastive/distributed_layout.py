...
def get_default_logger_fn(log_to_bigtable=False, log_every=10):
...
class DistributedLayout:
            ...
            for t in tqdm.trange(dataset["observations"].shape[0]):
                ...
                ts = dm_env.TimeStep(
                    ...
                    observation=np.concatenate(
                        [dataset["observations"][t], dataset["infos/goal"][t]]
                    ),
                )
                if t == 0 or dataset["timeouts"][t - 1]:
                    ...
                    adder.add(
                        action=dataset["actions"][t - 1], next_timestep=ts
    ...
    def actor(self, random_key, replay, variable_source, counter, actor_id):
        """The actor process."""
        adder = self._builder.make_adder(replay)
        ...
        return environment_loop.EnvironmentLoop(
            environment, actor, counter, logger, observers=self._observers
        )
    ...
    def build(self, name="agent", program=None):
        ...
        def make_actor(random_key, policy_network, variable_source):
        ...
        with program.group("evaluator"):
            for evaluator in self._evaluator_factories:
                ...
                program.add_node(
                    lp.CourierNode(
                        evaluator, evaluator_key, learner, counter, make_actor
                    )
                )
